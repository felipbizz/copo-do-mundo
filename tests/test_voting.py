...
def test_voting_form(voting_component, mock_session_state, mock_streamlit):
    ...
    assert (
        ...
        in st.session_state.rendered_elements
    )
    assert (
        "**Aparência**: Avalie a apresentação visual do drink" in st.session_state.rendered_elements
    )
...
def test_duplicate_vote_handling(voting_component, mock_session_state, mock_streamlit):
    ...
    assert (
        "Você já votou para o Participante #1 na categoria Categoria1!"
        in st.session_state.rendered_elements
    )
...
def test_voting_progress(voting_component, mock_session_state, mock_streamlit):
    ...
    total_possible_votes = SessionManager.get("num_participants") * len(
    ...
    assert (
        voting_component.vote_manager.calculate_progress(
